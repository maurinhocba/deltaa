 SUBROUTINE inpda1(task,ndime,neulr,nel,iwrit,elsnam,nelms)
 !******************************************************************
 !
 ...
 INTEGER (kind=4) :: nreqs,narch,nnode,nvare,j,nelem
 ...
 ELSE IF (TRIM(task) == 'RESTAR') THEN
   ...
   CALL resta1 ( ndime,nnode,nelem,nreqs,elset%head, elset%tail, elset%ngrqs,nvare)
   CALL add_ele01 (elset, head, tail)
