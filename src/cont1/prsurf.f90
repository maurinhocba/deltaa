...
!.... Open file por post-process
      IF(iwric > 0)THEN           !if contact forces are desired
        ...
        WRITE(41)npair,2          !number of forces and vector size
...
!.... WRITE contact surface for post-proccessing & compute mass
      ...
      DO isurf=1,nsurf              !for each surface
        ...
        IF( surf%iwrit == 1)THEN   !if visualization desired
          WRITE(18) nsegm,surf%sname  ! No of segments
          WRITE(18) 2             ! No of nodes per segment
          WRITE(18) ((surf%lcseg(i,j),i=1,2),j=1,nsegm) !connectivities
        END IF
...
!.... Update global flags BOTTOM and TOP
      opened = .FALSE.          !for surface pressure
      pair => headp             !point to first pair
      DO i=1,npair              !for each pair
...
!        END IF
        pair => pair%next       !point to next pair
      END DO
!      IF( opened ) WRITE(44)'NONAME',.FALSE.,.FALSE. !mark to end
