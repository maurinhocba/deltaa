MODULE cont1_db
  ...
  INTEGER (kind=4), PARAMETER :: &
     ...
     nrsdb = 6   ! number of real values for each slave node
  INTEGER (kind=4) :: &
     ntype, & ! 2-D problem
  ...
  REAL (kind=8) :: &
    oldis, & ! maximum displacement used in previous step
...
!** Derived type for contact 1 database **!
  ...
  TYPE pair1_db
    ...
    INTEGER (kind=4), POINTER :: issdb(:,:)  ! ISSDB(nisdb,ncnod)
               ...
               !(3) <0  not penetrated, =1 stuck  =2 sliding
    ...
    REAL (kind=8), POINTER :: rssdb(:,:) !RSSDB(nrsdb,ncnod)
                      ...
                      !(2) = vnx
                      !(3) = vny
                      !(4) = rp       onset local coord
                      !(5) = rpo      onset local coord (last converged)
                      !(6) = gap1     converged penetration
