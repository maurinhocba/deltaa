 ...
 REAL    (kind=8) s,t,q,sm,tm,zm,sp,tp,zp,z0, &
 ...
 ELSE IF (nnode == 12) THEN  !Bezier

   sm = 0.5d0*(1d0-s)
   sp = 0.5d0*(1d0+s)
   tm = 0.5d0*(1d0-t)
   tp = 0.5d0*(1d0+t)
   zm = 0.25d0*(1d0-2d0*q+q*q)
   z0 = 0.5d0*(1d0-q*q)
   zp = 0.25d0*(1d0+2d0*q+q*q)
   ...
   deriv(5,1) = -0.5d0*tm*z0
   deriv(6,1) = +0.5d0*tm*z0
   deriv(7,1) = +0.5d0*tp*z0
   deriv(8,1) = -0.5d0*tp*z0

   deriv( 9,1) = -0.5d0*tm*zp
   deriv(10,1) = +0.5d0*tm*zp
   deriv(11,1) = +0.5d0*tp*zp
   deriv(12,1) = -0.5d0*tp*zp
   ...
   deriv(5,2) = -0.5d0*sm*z0
   deriv(6,2) = -0.5d0*sp*z0
   deriv(7,2) = +0.5d0*sp*z0
   deriv(8,2) = +0.5d0*sm*z0

   deriv( 9,2) = -0.5d0*sm*zp
   deriv(10,2) = -0.5d0*sp*zp
   deriv(11,2) = +0.5d0*sp*zp
   deriv(12,2) = +0.5d0*sm*zp
