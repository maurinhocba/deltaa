...
REAL    (kind=8) :: fm,fl,stk,pr(nvarl)


!WRITE(58,"('matriz elemental')")
k = 0                                            !post. in local matrix
DO i = 1,nvarl                                   !for each column
  neci = lm(i)                                   !assoc. equation
  !WRITE(58,"(8e20.12)")locst(k+1:k+nvarl-i+1)
  IF(neci > 0) THEN                              !if active dof
    DO j = i,nvarl                               !for each row
      ...
      !IF(j == i .AND. stk == 0d0 )WRITE(58,"('null diagonal value ',i6,i8)")i,necj
