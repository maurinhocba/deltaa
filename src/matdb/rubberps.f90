 SUBROUTINE rubberps(pr,lb,model,stre,vec,mat,r1,r2)
 ...
 REAL(kind=8), OPTIONAL, INTENT(OUT) :: vec(:), & !elasticity (stiffness) matrix in vector form
                                        mat(:,:)  !elasticity (stiffness) matrix in matrix form
 ...
 REAL(kind=8)  :: &
             ...
             c(4),elas(6),    & ! local elasticity matrix
             r11,r22,r12,r1111,r2222,r1122,r1112,r1222  !auxiliar rotation terms
 ...
 IF( ci1 )THEN
   ...
   DO i=1,2
     d(1,i) = 2d0*lb(i)*(1d0-lb2(3)/lb2(i))          !I1,Li
   END DO
   IF( ci2 )THEN
     ! first derivatives of I2
     i2 = 1d0/lb2(3)+lb2(3)*(lb2(1)+lb2(2))         !I2
     DO i=1,2
       d(2,i) = 2d0/lb(i)*(1d0/lb2(3)-1d0/lb2(i))   !I2,Li
     END DO
   END IF
 END IF
 ...
 CASE (4:6) !ogden
   ...
   DO j=1,nterm                            !for each term
     ...
     k = k+2
   END DO
   bet = bet/lb(1:2)    !phi_i = beta/lambda
 ...
 bet = bet/lb(1:2)         !S = phi_i/lambda = beta/lambda^2

 !COMPUTE ELASTICITY MATRIX IF REQUIRED
 IF( PRESENT(vec) .OR. PRESENT(mat) ) THEN  !to compute stiffness

   IF( ci1 )THEN
     ! Second Derivatives of First Invariant
     DO i=1,2
       dd(1,i) = 2d0+6d0*lb2(3)/lb2(i)                !I1,Li,Li
     END DO
     dd(1,3) = 4d0*lb2(3)*lb(3)                       !I1,Li,Lj
     IF( ci2 )THEN
       !  Second Derivatives of Second Invariant
       DO i=1,2
         dd(2,i) = 2d0*(1d0/lb2(3)/lb2(i)+3d0/lb2(i)**2)  !I2,Li,Li
       END DO
       dd(2,3) = 4d0/lb(3)                                 !I2,Li,Lj
     END IF
   END IF
   ...
   CASE (1) !Arruda-Boyce
     ...
     a2 = pr(5)+i1*(2d0*pr(6)+i1*(3d0*pr(7)+i1*4d0*pr(8))) !Phi,I1,I1
     DO i=1,2
       c(i) = a1*dd(1,i) + a2*d(1,i)**2
     END DO
     c(3) = a1*dd(1,3) + a2*d(1,1)*d(1,2)

   CASE (2) !Mooney-Rivlin

     c(1:3) = pr(1)*dd(1,:) + pr(2)*dd(2,:)
   ...
   CASE ( 7: 9) !polynomial model
     ...
     END IF
     DO i=1,2
       c(i) = a1*dd(1,i) + a2*dd(2,i) &
            + a3*d(1,i)**2 + 2d0*a4*d(1,i)*d(2,i) + a5*d(2,i)**2
     END DO
     c(3) = a1*dd(1,3) + a2*dd(2,3)   &
          + a3*d(1,1)*d(1,2)+a4*(d(1,1)*d(2,2)+d(1,2)*d(2,1))+a5*d(2,1)*d(2,2)
   ...
   CASE (10:12) !Reduced polynomial
     ...
     DO i=1,2
       c(i) = a1*dd(1,i) + a3*d(i,1)**2
     END DO
     c(3) = a1*dd(1,3) + a3*d(1,1)*d(2,1)

   END SELECT

   c(1) = (c(1) - bet(1))/lb2(1)             !DS2/L2
   c(2) = (c(2) - bet(2))/lb2(2)             !DS1/L2 = DS2/L1
   c(3) =  c(3) * lb(3)

   IF( PRESENT(r1) .AND. PRESENT(r2))THEN
     ! compute shear term
     IF( ABS((lb(1)-lb(2))/lb(1)) < 1e-13 )THEN  !equal eigenvalues
       c(4) = (c(1) - c(3))                      !DS12/DE12
     ELSE
       c(4) = 2d0*( bet(1) - bet(2))/(lb2(1) - lb2(2))
     END IF
     ...
     elas(1) = c(1)*r1111 + c(2)*r2222 + (c(3)+c(4))*r1122*2d0     !(1,1)
     elas(2) = (c(1)+c(2)-2d0*c(4))*r1122 + c(3)*(r1111+r2222)     !(1,2)
     elas(3) = c(1)*r1112 - c(2)*r1222 - (c(3)+c(4))*(r1112-r1222) !(1,3)
     elas(4) = c(1)*r2222 + c(2)*r1111 + (c(3)+c(4))*r1122*2d0     !(2,2)
     elas(5) = c(1)*r1222 - c(2)*r1112 + (c(3)+c(4))*(r1112-r1222) !(2,3)
     elas(6) = (c(1)+c(2)-2d0*c(3))*r1122 + c(4)*(r1111+r2222-2d0*r1122)/2d0 !(3,3)
     IF( PRESENT(vec) )THEN
       vec(1:6) = elas
