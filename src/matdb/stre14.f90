 ...
 IF( plast )THEN
   ...
   IF(efpst > 0d0 )THEN   !if plastic step, update internal variables
     ...
     varin(4)   =  efpst ! + varin(nvar+1)  !increment only (implicit)
     pflag = .TRUE.
   END IF
 ELSE
   vms = SQRT(sigma(1)**2+sigma(2)**2-sigma(1)*sigma(2)+3d0*(sigma(3)**2+tau(1)**2+tau(2)**2))
   !varin(nvar+1) = 0d0      !elastic step
 END IF
...
!  REAL (kind=8), PARAMETER :: c1=1.154700538
  REAL (kind=8), PARAMETER ::  toler=1.05d-2, toler1=1d-2 !similar to bst
