 SUBROUTINE inpd26 (task, nel, iwrit, elsnam, nelms)

 !   READ control DATA for element number 26 (TL QUAD 4)
 ...
 TYPE (ele26_set), POINTER, SAVE  :: elset, anter

 IF (TRIM(task) == 'INPUT') THEN
   ! check if list of sets and set exists and initializes
   CALL srch_ele26 (head, anter, elset, elsnam, oldset)
   IF (oldset) THEN    !if set exists
     CALL comm26 (1, nelem,  nreqs, narch, elsnam, elset, nn, ngaus)
     nel = nel - nelem
   ELSE                !set ELSNAM does not exist
     ...
     elset%bbar  = .FALSE.
     elset%locax = 0
     CALL listen('INPD26')  !read a line
     nn = 12
     ...
     CALL ini_ele26e (elset%head, elset%tail)
     ALLOCATE( elset%gpc(3,ngaus))
   END IF
   !  read new data or data to previous data
   CALL elmd26(nelem, nn, elset%head, elset%tail, iwrit, ngaus, elset%shell)
   ...
   IF (.NOT.oldset) THEN
     CALL add_ele26 (elset, head, tail)
 ...
 !  elset%check = .FALSE.
 !  CALL ini_ele26e (elset%head, elset%tail)     !nullify head pointer
 ...
 !  CALL rest26 (nelem, nn,  nreqs, elset%head, elset%tail, &
 ...
 !  END IF
 !  ! restore list of elements
 !  CALL impo26 ( nelem,ngaus, elset%head, elset%tail, elset%shell)
 !  elset%check = .FALSE.
 !  ! add to list of elements
 !  IF( .NOT.overw )THEN
 !    CALL add_ele26 (elset, head, tail)
 ...
 ELSE
   CALL runend('INPD26: NON-EXISTENT TASK .        ')
 END IF
 CALL comm26(0, nelem,  nreqs, narch, elsnam, elset, nn, ngaus)

 RETURN
 9999 CALL runen2('')
 END SUBROUTINE inpd26
