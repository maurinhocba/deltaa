 ...
 USE ctrl_db,ONLY: ndime, ndofn, neulr, npoin, npoio, echo_chnode, nload, memo !, ndyna
 ...
 IMPLICIT NONE
   ...
   IF( TRIM(actio) /= 'NEW' )THEN
     ...
     DO n = 1, npoin                !for each existing point
       ...
       END IF
!      must be rewritten for velocities
!      IF (ndyna > 0) THEN
!        DO idofn = 1,ndofn
...
!      END IF
     ...
     DEALLOCATE ( coora, coorc, coord, resid, label) !release mem
     IF( ASSOCIATED (eule0 ))DEALLOCATE ( eule0) !release mem
     IF( ASSOCIATED(coor1) )DEALLOCATE(coor1)
     IF( neulr > 0 )THEN
   ...
   DO is=1,nimpf            !  GLOBAL LOOP over each imported element set
     ...
     ELSE  !if new nodes are included
       ...
       !IF( i > memo) CALL runend('IMCOOR: Max. number of nodes EXCEDED !!!!!!')
       DO i=1,np                          !loop over all the nodes
         n = n+1
         READ(nf)nn,x(1:nv)               !node label and coordinates
         IF( displ ) READ(nf) d(1:ndime)  !displacements
         IF(iwrit==1)WRITE(lures,"(i10,9f10.5)",ERR=9999)  &
             nn,x(1:ndime),x(ndime+1:nv)/facto
         IF(ieulr == 1) THEN                 !EULER angles in a 2-D problem
           rm(1:2) = x(3:4)                  !angle (rad) is the third parameter
         ...
         END IF
         !IF( ndyna > 0)nd%veloc(1:ndofn) = 0d0             !initializes velocity
   ...
   ELSE IF( chlab )THEN  !for subsequent strategies
     ...
     !CALL contac ('UPDLON')               ! CONTACT nodes (CONT 2 & 3 only)
     echo_chnode = .TRUE.
     DEALLOCATE (oldlb)
