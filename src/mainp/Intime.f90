...
!
!***********************************************************************
      USE ctrl_db, ONLY: ndime,nrotd,ndofn,neulr,npoin,initial_displacements, &
                         initial_rotations,ndyna,ndoft,itemp
      USE npo_db, ONLY: label,eulei,coorc,coora,tempe,dtemp,psia,velnp
      USE outp_db, ONLY : iwrit
      USE c_input
      USE nsets_db

      IMPLICIT NONE
      REAL(KIND=8), PARAMETER  :: facto = 57.295779513082320876798154814105      ! facto= 180d0/pi
      REAL    (kind=8) :: xg(9)
      INTEGER (kind=4) :: i,n, neu,na
   ...
   DO                             !loop for each type of variables
     ...
     IF (exists('DISPLA')) THEN   !key word DISPLACEMENTS
       ...
       IF( .NOT.initial_displacements )THEN
         ...
         coorc = 0d0
         IF(ndofn == 8 .OR. ndofn == 4 ) psia = 0d0
       END IF
                     !print header
...
&                        '   Initial Y-disp.   Initial Z-disp.'/)",ERR=9999)
       ...
       DO                               !loop for nodal displacements
         ...
         ELSE       ! displacements of a single node
           ...
           coorc(1:ndime,i) =  xg(1:ndime) !store to be summed later
           IF(iwrit == 1) WRITE(lures,"(i10,3e24.13)",ERR=9999) label(i), &
                          xg(1:ndime)
         END IF
         CALL listen('INTIME')
       END DO
     ...
     IF (exists('LOCALS')) THEN    !key-word LOCAL_SYSTEMS
       ...
       ! initial rotations are applied at INITIAL_COMP/WRTPOS/ACTCD
       IF( .NOT.initial_rotations )THEN
         initial_rotations = .TRUE.
         IF(.NOT.ASSOCIATED(eulei)) ALLOCATE(eulei(3,npoin))
         eulei = 0d0 !initializes
       END IF
       neu = 2*ndime - 3
       ...
       DO                          !loop for nodal local systems
         ...
         IF (found) THEN                   !IF set found
           ...
           DO                              !loop over the node
             ...
             eulei(1:neu,i) = xg(1:neu)/facto  !associate data
             IF(iwrit == 1) WRITE(lures,"(i7,3x,(3f20.13))",ERR=9999) &  !ECHO
                            label(i),xg(1:neu)
     ...
     IF (exists('ADDISP')) THEN    !key-word AD_DISP
       ...
       ELSE IF(ndofn == 4) THEN         !
         IF(iwrit == 1)WRITE(lures,"(//,'   Node       Add-DoF '/)",ERR=9999)
         na = 1
       ELSE                        ! error
         ...
         IF (exists('SET   ',i))THEN       !data for a SET
           ...
           IF (found) THEN                   !IF set found
             ...
             DO                              !loop over the node
               ...
               n = get_label(ns)             !nodal label
               i = chnode(n)                 !internal number
               psia(1:na,i) = xg(1:na)       !associate data
     ...
     IF (exists('VELOCI')) THEN   !key-word VELOCITIES (all DOFs)
       IF( ndyna == 0 )CALL runend('INTIME: static problems, velocities not possible')
         ...
         IF (exists('SET   ',i))THEN  !If data is for a SET
           ...
           IF (found) THEN            !if set found
             ...
             DO                       !loop over the nodes
               ...
               i = chnode(n)    !internal number
               !velnp(1:nrotd,i) = xg(1:ndofn)  !associate data
