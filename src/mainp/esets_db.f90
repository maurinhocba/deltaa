 MODULE esets_db
   ...
   INTEGER (kind=4), PARAMETER :: msets = 30 ! number of element types
   ...
   CHARACTER(len=midn), PARAMETER :: ELTY(msets) = (/ &
            'SPOT  ','TRUSS ','TLLL  ','SSOLAG','SPRISM',  &
            'SHELQ ','SHELT ','BEAM  ','SHREV ','RIGID ',  &
            'BEAM2 ','SOLSH ','NBST  ','LBST  ','RBST  ',  &
            'PRISM ','QUADL ','SOLAG ','BTRIAN','TR2D  ',  &
            'NONE  ','NONE  ','NONE  ','NONE  ','BSQ   ',  &
            'BSOLAG','BPRISM','NONE  ','QPRISM','BBST  '  /)
   INTEGER(kind=4), PARAMETER :: mnode(msets) = (/ &
                     2, 2, 9, 8,12, 4, 6, 3, 3, 8, &
                     8,12,18, 6, 6,18, 8,20, 6, 6, &
                     0, 0, 0, 0,12,12,12, 0,15,12 /)
   ...
   CHARACTER (len=mset), PARAMETER :: snames(7)= (/ 'node set',        & !(1)
                                                    'element set',     & !(2)
                                                    'velocity set',    & !(3)
                                                    'contact pair',    & !(4)
                                                    'contact surface', & !(5)
 ...
 CONTAINS
   ...
   SUBROUTINE elsets ( )
     ...
     DO iset=1,msets    !for each element type
         ...
         END IF
     END DO
     rot_free = (nelms(11) + SUM(nelms(13:15)) + SUM(nelms(24:25))+nelms(30)) > 0     !true if rotation free elements exist
