 ...
 USE npo_db, ONLY : label,coord,coora,euler,ifpre,iffix,eule0,naeul,nodset,numpo,auxi
 USE mat_dba
 ...
 DO is=1,neset ! loop for each Element Set to Export
   ...
   IF( matsc ) THEN
     ...
     IF( .NOT.ASSOCIATED(auxi) )ALLOCATE(auxi(100))
     auxi = -1   !initializes to none
     CALL elemnt ('SECDAT', name=name, flag2=found ) !sections => AUXI
