 SUBROUTINE beginp(actio,nrest,memo,cpui,kstep)
!*********************************************************************
!
...
!*********************************************************************
 ...
 USE c_input,ONLY : openfi, upcase
 USE kinc_db, ONLY : nn
 ...
 IMPLICIT NONE
   ...
   INTEGER (kind=4),INTENT (OUT) :: nrest,  & ! time between dumping of restar files [sec]
                                    memo,   & ! memory for auxiliar arrays
   ...
   INTEGER(kind=4) :: narg,i,ichek,lengs(npar),numpar(npar),j,k,number,lng
...
!-------------------set defaults
   ...
   nrest = 0        !do not generate restart file
...
!------------------- IF no arguments use COMAN.DAT file
   ...
   ELSE      !-------------------more than one argument
     ...
     IF (error(8)) THEN    !restart from last restart file
       !lng = INDEX(input,'.dat',BACK = .TRUE.) - 1
       !namr = input(1:lng)
       namr = TRIM(output)
