 ...
 USE ctrl_db,ONLY:  ndime, ndofn, nrotd, neulr, npoin, npoio, echo_chnode, ndyna!, nload
 USE outp_db, ONLY : iwrit !,updlon_outp
 USE c_input
 USE ndinf_db
 USE nsets_db
 USE esets_db, ONLY : add_name
 USE npo_db !, ONLY: label,oldlb,euler,coord,coorc,coora,resid,scale
 ...
 IMPLICIT NONE
   ...
   REAL (kind=8) :: fi, raddi, rm(9), theta, x(6)
   ...
   IF (TRIM(actio) /= 'NEW') THEN
     ...
     DO n = 1, npoin                !for each existing point
       ...
       END IF
       IF(ndyna > 0)nd%veloc(1:ndofn) = velnp(1:ndofn,n) !store nodal velocities
     ...
     END DO
     DEALLOCATE ( coora, coorc, coord, label) !release mem
     IF(ndyna > 0) DEALLOCATE ( velnp) !release mem
     IF( ASSOCIATED(coori) )DEALLOCATE(coori)
     IF( ASSOCIATED(emass) )DEALLOCATE(emass)
     IF( neulr > 0 )THEN
       IF( ASSOCIATED(eule0) )DEALLOCATE(eule0)
       IF( ASSOCIATED(euler) )DEALLOCATE(euler)
       IF( ASSOCIATED(locsy) )DEALLOCATE(locsy)
       IF( ASSOCIATED(eule1) )DEALLOCATE(eule1)
       IF( ASSOCIATED(locs1) )DEALLOCATE(locs1)
     END IF
     ...
     IF(exists('SET   ',k))THEN          !check if set control card read
       ...
       DO
         ...
         IF( iwrit == 1) THEN                !If echo
           ...
           CASE (0)
             WRITE(lures,"(i10,3E15.5)",ERR=9999) nn,param(2:ndime+1)
           CASE (1)
             WRITE(lures,"(i10,2E15.5,10x,f10.5)",ERR=9999) nn,param(2:4)
           CASE (9)
             WRITE(lures,"(i10,6E15.5)",ERR=9999) nn,param(2:7)
           END SELECT
         END IF
         ...
         nd%veloc(1:nrotd) = 0d0              !initializes velocity
   ...
   IF( (npoin <= max_npoin_3d .AND. ndime == 3 ) .OR. (npoin <= max_npoin_2d .AND. ndime == 2 )) THEN
     ALLOCATE (coord(ndime,npoin), coora(ndime,npoin), coorc(ndime,npoin),  &
               label(npoin),coori(ndime,npoin))
     IF( ndofn == 8 )THEN
       ALLOCATE( psia(2,npoin),psic(2,npoin),psii(2,npoin))
       psia = 0d0
       psic = 0d0
       psii = 0d0
     ELSE IF( ndofn == 4 )THEN
       ALLOCATE( psia(1,npoin),psic(1,npoin),psii(1,npoin))
       psia = 0d0
       psic = 0d0
       psii = 0d0
     ELSE IF( ndofn == 7 )THEN
       ALLOCATE( psia(4,npoin),psic(4,npoin),psii(4,npoin))
       psia = 0d0
       psic = 0d0
       psii = 0d0
     ELSE
       NULLIFY( psia ,psic, psii )
     END IF
     IF(ndyna > 0) ALLOCATE ( velnp(ndofn,npoin)) !get mem for nodal velocities
   ELSE
     STOP ' number of nodes exceeded for Academic version'
   END IF

   IF(neulr > 0)THEN                   !IF nodal systems
     ALLOCATE(eule0(neu,npoin), euler(neulr,npoin), locsy(neulr,npoin))      !get memory
     euler = 0d0                       !initializes
   ELSE
     ALLOCATE( euler (1,1), locsy(1,1) )           !to avoid a null pointer
   END IF
   ...
   DO n=1,npoin                        !for each node in the list
     ...
     coori(1:ndime,n) = coora(1:ndime,n)
     IF(ndyna > 0)velnp(1:ndofn,n) = nd%veloc(1:ndofn)
     IF (neulr > 0) THEN
