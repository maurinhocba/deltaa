SUBROUTINE fixval(iwrit,ndime,ndofn,ifpre,nvfix,rot_free,iffix,label)
  ...
  DO n=1,nifx                !for each node in the list
    ...
    !    END IF
    !  END IF
    !ELSE IF( ndofn == 4 )THEN      !for Psi functions
    !  IF ( ifpre(4,ipoin) == 0 )THEN     !if the DOF exist
    !    IF(ifix(3) == 1) THEN          !if a restriction is included
    !      nvfix = nvfix+1                !increase number of fixed values
    !      ifpre(4,ipoin) = -(nn+nvfix)   !assign a position
    !    END IF
    !  END IF
    !ELSE IF( ndofn == 7 )THEN      !for Psi & additionsl DOFs functions
    !  DO i=4,ndofn
    !  IF ( ifpre(i,ipoin) == 0 )THEN     !if the DOF exist
    !    IF(ifix(3) == 1) THEN          !if a restriction is included
    !      nvfix = nvfix+1                !increase number of fixed values
    !      ifpre(i,ipoin) = -(nn+nvfix)   !assign a position
    !    END IF
    ...
    ifx => ifx%next                     !point to next node
  END DO
 ! DO ipoin=1,npoin             !nullify extra DOF for a test
 !   IF( ifpre(4,ipoin) == 0 )THEN
 !      nvfix = nvfix+1                !increase number of fixed values
 !      ifpre(4,ipoin) = -(nn+nvfix)   !assign a position
 !   END IF
 ! END DO
