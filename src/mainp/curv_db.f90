...
!  END SUBROUTINE dalloc_cur
   ...
   SUBROUTINE set_cur_cte (i,lambd)

     !   modifies a curve into a constant one

     !USE c_input, ONLY : runend, lures
     IMPLICIT NONE

     INTEGER (kind=4), INTENT(IN) :: i         ! curve position
     REAL(kind=8), INTENT(IN) :: lambd         ! present curve value

     TYPE(curpar),POINTER :: cur
     INTEGER (kind=4) :: j         ! index

     cur => headcp       !point to first curve
     DO j=1,i-1          !loop to the curve
       cur => cur%next          !point to next curve
     END DO

     DEALLOCATE (cur%vec)       !release memory for parameters
     cur%dim    = 5             !storage space
     ALLOCATE (cur%vec(cur%dim))   !memory for parameters
     cur%vec(1) = REAL(1,8)     ! type of curve
     cur%vec(2) =  1d0  ! type of curve FORCE
     cur%vec(3) = 0d0   ! start time
     cur%vec(4) = 1d9   ! Ending Time
     cur%vec(5) = lambd ! Function Value

     RETURN

   END SUBROUTINE set_cur_cte
