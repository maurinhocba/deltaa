 SUBROUTINE elem05(task , elsnam, ttime,iload,iforce,              &
 ...
 INTEGER (kind=4) :: nreqs, narch, nnode, nelem, newel, ngaus
...
!     CALL close1(nreqs,narch)
   ...
   CASE ('MASMTX')
     CALL masm05(ndime,elset%head,emass,mass,sumat,flag1,ngaus,ifpre)

   CASE ('OUTPUT')      !output variables for post-processing

     IF(flag1.OR.flag2) CALL outp05 (flag1,flag2, iwrit, &
            elset%head, nreqs, narch, elset%ngrqs, ttime, ngaus, elset%isg,elset%psg)

   CASE ('RESVPL')      !compute internal nodal forces
     CALL resv05 (elset%head, coora, resid, istop, ttime, elset%small, ngaus, &
                  quad, nnode)
   ...
   CASE ('STIFFM')
     CALL stif05(elset%head, gstif, resid(:,iforce), coora, elset%small, &
                 ngaus, quad, nnode)
...
!
   ...
   !    CALL smth05(coord,coora,strnd,elset%head,iffix,npoin, &
   ...
   !    CALL nods05 (npoin, nelem, numpo, nodset, elset%head, label, l_old)
   !    EXIT
   !  END IF
   ...
   !    CALL inig05 (elset%head,nelem,iwrit,elset%plstr)
   !    EXIT
   !  END IF
