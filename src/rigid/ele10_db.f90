 ...
 CONTAINS
   ...
   SUBROUTINE new_ele10(elset)
   !Create a new element of ELE10 sets
     ...
     elset%nnode = 0        !     "     number of nodess
     elset%rbnod = 0        !     "     Rigid Body NODe (label of master node)
     elset%nmast = 0        !     "     master node internal number
     elset%tmass = 0d0      !     "     total Translational mass
     elset%ntype = 0        !     "     problem type
     NULLIFY(elset%lnods,elset%matno)
     NULLIFY(elset%next)

   RETURN
   END SUBROUTINE new_ele10
