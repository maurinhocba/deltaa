...
IMPLICIT NONE
!     arguments
TYPE (pair4_db), POINTER :: p  !INTENT(OUT)
...
IF (exists('SLAVE ',j)) THEN            !check Key-word
  IF( param(j) == 0)THEN                !if no associated number
    ...
    surpos  = words(j+2)(1:6)           !pos. (top center reversed bottom)
  ELSE                                  !label is a two digit number
    p%slave = '    '//TRIM(inttoch(INT(param(j)),2)) !fill with blanks
    surpos  = words(j+1)(1:6)           !pos. (top center reversed bottom)
  END IF
  SELECT CASE (surpos)
  CASE ('BOTTOM')
    p%slsur = -2
...
IF (exists('MASTER',j)) THEN           !check Key-word
  ...
  ELSE                                 !label is a two digit number
    p%master = '    '//TRIM(inttoch(INT(param(j)))) !fill with blanks
...
p%gapinc=-getrea('GAPINC',-p%cutoff/20,' Maximum Incremental Gap per Step..')
...
IF( p%press ) WRITE(lures,"(15x,'Contact pressure will be computed')")

p%wrink = exists('WRINKL')
IF( p%wrink ) WRITE(lures,"(15x,'Wrinkle control will be performed')")


p%prev = .FALSE.  !not existence in previous step
WRITE(lures,"(//)")
RETURN
END SUBROUTINE cspin4
