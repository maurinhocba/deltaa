...
DO icnod = 1, ncnod
  ...
  IF(nearn == 0 .OR. nnp > freq ) THEN   !if no previous or too many steps
    ...
    CALL nears4(xc,nsegm,nearn,coors(:,isn))
  ...
  CALL projt4(coors(:,isn),nearn,coorm,lcseg,nhseg, &
              issdb(:,icnod),rssdb(:,icnod),cprop(4),cprop(5),curms,cu)
  IF( issdb(4,icnod) == 0 ) CYCLE   ! no penetration

  !.... step 3 .... Compute contact force

  CALL force4(isn,issdb(:,icnod),rssdb(:,icnod),lcseg,indco,cprop, &
