...
!**************************SHELL2*************************************************
   IF( secty == 12 )THEN              !for standard solid section
     IF( ttti )THEN
       ...
       iF(.NOT.elast)large = large .OR. ASSOCIATED(e%gausv)  !use log strains
       DO l=1,nlayr                        !for each layer
         ...
         CASE ( 5 )     !orthotropic material
           ...
           stres(3)= c(4)*stran(3)
           IF( plast) THEN
             CALL corr05(stres(1),stres(2),stres(3),varin(8,l),c(:),propi(:),ierr, &
                         varin(5:7,l),aux)
             IF ( varin(8,l) > 0.d0 )  pflag(l)=.TRUE.
           END IF
         ...
         CASE ( 6 )  ! Hyperelastic
           ...
           !  IF( efpst > 0.0 )THEN !if the material has unrecoverable strains
           ...
           !    varin(8,l) = efpst      !effective plastic strain
           !    varin(5:7,l) = stran(1:3) - dstpl(1:3)   !plastic  log strains
           !  END IF
           !
           !END IF
           aux = SQRT(stres(1)**2+stres(2)**2-stres(1)*stres(2)+3d0*stres(3)**2)
