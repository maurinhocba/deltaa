 SUBROUTINE inpda2(task,ndime,nel,iwrit,elsnam,nelms)
 !******************************************************************
 !
...
&                    nelms      ! number of element sets of this type
 ...
 IF (TRIM(task) == 'INPUT') THEN
   ...
   END IF
   CALL elmda2(ndime,nelem,elset%head,elset%tail,iwrit)
 ...
 ELSE IF (TRIM(task) == 'RESTAR') THEN
   ...
   CALL resta2 ( ndime,nelem,nreqs,elset%head, elset%tail,elset%ngrqs)
   CALL add_ele02 (elset, head, tail)
