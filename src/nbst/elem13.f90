 SUBROUTINE elem13(task , elsnam, ttime,iload,iforce,              &
 ...
 DO
   ...
   CASE ('NEW   ','NSTRA1','NSTRA2')
     IF( .NOT.elset%lside )WRITE(55,"(' extended connectivities for set ',a)")elset%sname
     CALL acvd13 (ifpre, elset%head, elset%lside, &
                  nelem, elset%nbs, elset%bhead)


   CASE ('ACTUAL')
     CALL actu13(elset%head)
   ...
   CASE ('LOADPL')
     CALL load13 (igrav, loadv(:,:,iload), gv, gravy, elset%head)
   ...
   CASE ('MASMTX')
     CALL masm13(ndime,nnode,elset%head,emass,mass,sumat,flag1,ifpre)

!   CASE ('OUTDYN')
!     CALL outd13(nelem,m(nmatno),m(nindex),gprop(1),m(ngausv),nquad,
!  &              sname)

   CASE ('OUTPUT')
      IF(flag1.OR.flag2) CALL outp13 (flag1,flag2,elset%logst, iwrit, &
   ...
   CASE ('STIFFM')
     CALL stif13(elset%head,coora,gstif,resid(:,iforce),logst, elset%stint,iffix)
   ...
   CASE ('NODSET')      !compute nodes set from element set
     ...
     IF (flag2) THEN
       ...
       CALL nods13(nelem, elset%head, label)
       EXIT
     END IF
   ...
   CASE ('SECDAT')
     ...
     IF (flag2) THEN
       ...
       CALL secd13 (elset%head,elset%nelem,auxi)
       EXIT
     END IF
