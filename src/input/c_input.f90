...
!
     CASE (10)  !Auxiliar ASCII file
       OPEN(10,FILE=fpath(1:fpatn)//TRIM(output)//TRIM(rest),FORM='formatted',STATUS=st_key)
     ...
     CASE (47:49) !Any auxiliar file
       OPEN(nfile,FILE=fpath(1:fpatn)//'aux',FORM='unformatted',STATUS='scratch')
     ...
     CASE (50)  ! Dumping Files for restart
       ...
       ELSE IF (flag == 1) THEN  !Restart file at the middle of the strategy
         ...
         oflag = 0
         !Copy output file *.f16 from restart
         ...
         DO i=11,15
            !copy output file *.p?? from restart
            dummy = if_exist_copy_file( &
     ...
     CASE (51)  !Files for restart (read)
       ...
       ELSE
         ...
         !Copy output file *.fc4 from restart
         dummy = if_exist_copy_file( fpath(1:fpatn)//TRIM(rest)//'.fc4 ',         &
                            fpath(1:fpatn)//TRIM(string)//'.fc4'         )

         !Copy output file *.fw4 from restart
     ...
     CASE (55)  !report file
       OPEN(55,FILE=fpath(1:fpatn)//TRIM(output)//'.rep',FORM='formatted',              &
            ACCESS='SEQUENTIAL',STATUS='unknown',SHARED)
     ...
     CASE (59)  !file for SHREV profile stresses and diplacement (temporary)
       OPEN(59,FILE=fpath(1:fpatn)//TRIM(output)//'_t.dat',FORM='formatted',              &
            ACCESS='SEQUENTIAL',STATUS='unknown',SHARED)
