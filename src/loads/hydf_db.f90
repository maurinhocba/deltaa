 ...
 !***********************************************************************
 SUBROUTINE hydflo (hydfl, lc, factor, ttime)
 ...
 REAL (kind=8), INTENT (IN) :: factor,ttime
 !--- Function
 ! local
 INTEGER (kind=4) :: n, ie, in, ng, node, nnode
 REAL (kind=8)    :: facts(2), pforc, press, &
                     px(3), t(3,2), vn(3), x(3,4)
 ...
 press = hydfl%press * facts(1)    !fluid press
 ...
 DO ie = 1, hydfl%nelem
   ...
   IF ( nnode == 3) THEN                   !triangles
     t(1:3,1) = x(1:3,2) - x(1:3,1)    ! side 3
     t(1:3,2) = x(1:3,3) - x(1:3,1)    !-side 2
     !*** cross product  T3 = T1 x T2 = 2 area * t3
     vn(1) = t(2,1)*t(3,2) - t(3,1)*t(2,2)
     vn(2) = t(3,1)*t(1,2) - t(1,1)*t(3,2)
     vn(3) = t(1,1)*t(2,2) - t(2,1)*t(1,2)
     pforc = press/2.d0/nnode
     px = pforc*vn
   ELSE IF ( nnode == 4) THEN              !quads
     t(:,1) = (-x(:,1)+x(:,2)+x(:,3)-x(:,4))/2d0
     t(:,2) = (-x(:,1)-x(:,2)+x(:,3)+x(:,4))/2d0
     !*** cross product  T3 = T1 x T2 = area * t3
     vn(1) = t(2,1)*t(3,2) - t(3,1)*t(2,2)
     vn(2) = t(3,1)*t(1,2) - t(1,1)*t(3,2)
     vn(3) = t(1,1)*t(2,2) - t(2,1)*t(1,2)
     pforc = press/nnode
...
!      END SUBROUTINE sortnd
 ...
 DO k = 1, numps
   ...
   !  ads.gt.0 slave node lies to the right of closest master node
   !  ads.lt.0 slave node lies to the  left of closest master node
