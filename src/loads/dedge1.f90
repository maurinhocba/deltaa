...
&   5x,'List of Loaded Edges and Applied Loads'/)",ERR=9999) nedge
 ...
 DO iedge = 1,nedge  !for each loaded edge
   ...
   IF( ecpx )THEN  !if Bezier triangles exist
     IF( nn == 3 .AND. cpx(1,noprs(2)) /= 0 ) THEN !if this is a Bezier triangle side
       !extra check could be done comparing data 2:3 with the other nodes
       weigh = (/ 5.d0,8.d0,5.d0 /)/18.d0
       shape(:,:) = RESHAPE( (/ 0.787298335D0,  0.2D0,  0.012701665D0, &
                                0.25D0,         0.5D0,  0.25D0,        &
                                0.012701665D0,  0.2D0,  0.787298335D0 /),(/3,3/))
       deriv(:,:) = RESHAPE( (/ -1.77459667D+00, 1.54919334D+00, 2.25403331D-01, &
                                -1.00000000D+00, 0.00000000D+00, 1.00000000D+00, &
                                -2.25403331D-01,-1.54919334D+00, 1.77459667D+00 /),(/3,3/))
       press(:,2) = 2d0*press(:,2) - press(:,1)/2d0 -press(:,3)/2d0
     ELSE
