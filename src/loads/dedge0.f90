...
IMPLICIT NONE
  ...
  ELSE
    ...
    IF( exists('NNODE ',i))THEN
      nnode = INT(param(i))
      IF (iwrit==1) WRITE(lures,"(//5x,'Number of nodes per edge NNODE:',i3/)",ERR=9999)nnode
    ELSE
      nnode = 2
    END IF
    IF (exists('L     ')) THEN
      defsys = 'L'
      IF (iwrit==1) WRITE(lures,"(//5x,'Cartesian system por Loads: Local'/)",ERR=9999)
    ELSE IF (exists('G     ')) THEN
      defsys = 'G'
      IF (iwrit==1) WRITE(lures,"(//5x,'Cartesian system por Loads: Global'/)",ERR=9999)
    ELSE
      defsys = 'L'
    END IF
