...
&  'Applied loads'/)",ERR=9999)
 ...
 srf => heads                  !point to first segment
 DO isurf = 1,nsurf            !for each loaded segment
   systm = srf%systm3          !"L"ocal or "G"lobal
   nn = srf%nn3                !number of nodes defining segment
   noprs(1:nn) = srf%lnod3(1:nn)  !connectivities (original labels)
   nvert = nn                  !number of vertex nodes
   IF( nn > 4) nvert = nn/2
   DO i=1,nn
     noprs(i) = chnode(noprs(i))  !change labels to internal numeration
   END DO
   direc = 0d0                 !load direction initialization ¿?
   direc(1:ndime) = srf%direc3(1:ndime)  !load direction
   CALL vecuni(ndime,direc,area)         !unit vector
   IF (area == 0d0) direc(3) = 1d+0      !if null vector use default
   p(1:nn) = srf%press3(1:nn)            !nodal pressure at each node
   !       prints actual DATA
   IF(iwrit == 1) THEN
...
&         ,3f6.3,' Nodes',6i6)",ERR=9999) isurf,systm,nn,direc,noprs(1:NN)
   ...
   END IF
   rload = 0d0          !initializes load vector
   x(1:ndime,1:nn) = coord(1:ndime,noprs(1:nn)) !nodal coordinates
   SELECT CASE (nn)     !according to the number of nodes
   CASE (3)
   ...
   CASE (6)
     ...
     IF( ASSOCIATED(cpx)) THEN
       DO n=4,6
         IF( cpx(1,noprs(n)) /= 0) bezier = .TRUE.
       END DO
     END IF
     IF( bezier )THEN
       posgp(1:6) = (/ 0.5d0, 0d0, 0.5d0, 0.5D0, 0d0, 0.5d0 /)
       DO g = 1,3
         CALL shap07(posgp(2*g-1),posgp(2*g),shape(1),deriv(1),0)
         dx(1:ndime) = MATMUL(x(1:ndime,1:6),deriv(1:6))
         dy(1:ndime) = MATMUL(x(1:ndime,1:6),deriv(7:12))
         CALL vecpro(dx,dy,dz)
         CALL vecuni(ndime,dz,area)
         area = area/6d0
         ! direction of the load
         ...
         auxv = DOT_PRODUCT(shape(1:6),p(1:6))*area
         DO n = 1,nn
           fc = auxv*shape(n)
           rload(1:ndime,n) = rload(1:ndime,n) + fc*gldir
         END DO
