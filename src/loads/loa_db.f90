 MODULE loa_db
  ...
  TYPE loa_nod
    ...
    REAL (kind=8) :: forc(8)           !point force and moment components
  ...
  TYPE loa_set
    ...
    LOGICAL :: internal            !if initial internal forces are used as load
...
CONTAINS
  ...
  IF (nload > 0) THEN
    ...
    DO i=1,nload
      ...
      WRITE (50,ERR=9999) loas%numfl, loas%fltype, loas%flpar, loas%fluid, loas%internal
      seg => loas%headf
      DO j=1,loas%numfl
  ...
  IMPLICIT NONE
    ...
    DO i=1,nload
      ...
      READ(51) loas%numfl, loas%fltype, loas%flpar, loas%fluid, loas%internal
      DO j=1,loas%numfl
        CALL new_foll(seg)
